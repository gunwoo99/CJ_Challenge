...
class CJ_Cluster:
    def __init__(self, order_list, distance_matrix, vertex_to_index, terminal_site):
        self.order_number         = len(order_list) # 주문 수
        self.orders_site          = [] # 주문의 위치
        orders_destination        = [] # 주문의 착지 ID
        same_site_orders_indices  = [] # 동일한 위치의 주문
        ...
        self.clustered_orders = sorted(self.clustered_orders, key =lambda x:x[1]) # 거리로 정렬
        self.clustered_orders, self.distance, self.assigned_cluster_site_num = zip(*self.clustered_orders)
